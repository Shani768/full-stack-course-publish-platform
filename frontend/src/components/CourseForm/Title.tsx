...
const Title: React.FC<TitleProps> = ({ courseId, initialTitle }) => {
  ...
  const onSubmit = async (data: TitleFormData) => {
    try {
      setLoading(true);
      await axios.patch(`http://localhost:5000/api/courses/${courseId}`, data, {
        headers: {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex gap-4">
            <button
              ...
              onClick={() => {
                setIsEditingTitle(false);
                setValue("title", initialTitle); // Reset to original on cancel
              }}
